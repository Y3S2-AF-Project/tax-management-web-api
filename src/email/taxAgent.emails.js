...
export const sendGeneratedPassord = async (taxAgentObj, autoGeneratedPassword) => {
    ...
    const email = {
        body: {
            name: `${taxAgentObj.firstName} ${taxAgentObj.lastName}`,
            intro: `You have been added as an Tax Agent to the Tax Management System. We have generated a temporary password for you to access your taxAgent profile.`,
            action: {
                instructions: `Your temporary password is:<br><h1>${autoGeneratedPassword}</h1>`,
                button: {
                    color: '#22BC66',
                    text: 'Go to TaxAgent Dashboard',
                    link: 'http://localhost:3000/taxAgent/login/'
                }
            },
    ...
    await mailTransporter
        .sendMail(details)
        .then(() => {
            logger.info(`Email sent to ${taxAgentObj.email}`)
        })
        .catch((error) => {
...
const TaxAgentEmailService = {
    sendGeneratedPassord
}

export default TaxAgentEmailService
